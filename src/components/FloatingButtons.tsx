import { MessageCircle, Scale, ChevronUp } from 'lucide-react';
...
const FloatingButtons = () => {
  ...
  return (
    <>
      <div className="fixed bottom-24 md:bottom-8 right-4 md:right-8 z-20 flex flex-col space-y-3">
        <a
          href="https://wa.me/256783277052"
          target="_blank"
          rel="noopener noreferrer"
          className="w-12 h-12 md:w-14 md:h-14 bg-green-500 rounded-full flex items-center justify-center shadow-lg hover:shadow-xl transition-all hover:scale-110"
          title="WhatsApp Us"
        >
          <svg
        ...
        {showBackToTop && (
          <button
            onClick={scrollToTop}
            className="w-12 h-12 md:w-14 md:h-14 bg-[#0F52BA] rounded-full flex items-center justify-center shadow-lg hover:shadow-xl transition-all hover:scale-110 animate-bounce"
            title="Back to Top"
          >
      ...
      {compareList.length > 0 && (
        <div className="fixed right-0 top-1/2 -translate-y-1/2 z-20">
          <div className="bg-[#FFD700] text-[#0a0a0a] px-3 py-2 rounded-l-lg shadow-lg">
